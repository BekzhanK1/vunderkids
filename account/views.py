...
from account.serializers import (
    ClassSerializer, UserSerializer, StaffRegistrationSerializer, SupervisorRegistrationSerializer,
    ParentRegistrationSerializer, SchoolSerializer, StudentRegistrationSerializer,
    StudentSerializer, StudentsListSerializer, ChildrenListSerializer, ChildSerializer,
    SimpleStudentSerializer, MyTokenObtainPairSerializer
)
...
from .tasks import send_activation_email, send_mass_activation_email, send_password_reset_request_email
    ...
    def get(self, request, token):
        try:
            ...
            if user.is_activation_token_expired:
                ...
                return Response('Your activation link has expired! You will receive a new email soon', status=status.HTTP_403_FORBIDDEN)
            user.is_active = True
...
class RequestResetPassword(APIView):
    ...
    def post(self, request):
        ...
        if email:
            ...
            return Response({"message": "You need to enter the email to reset the password"}, status=status.HTTP_400_BAD_REQUEST)
...
class SchoolViewSet(viewsets.ModelViewSet):
    ...
    def deassign_supervisor(self, request, pk=None):
        ...
        school.save()
        return Response({"message": "Supervisor has been removed from the school"}, status=status.HTTP_200_OK)
    ...
    def process_excel(self, excel_file):
        ...
        school, created = School.objects.get_or_create(
            name=school_name,
            defaults={'city': city, 'email': school_email}
        )
...
class SupervisorSchoolViewset(viewsets.ReadOnlyModelViewSet):
    ...
    def students_of_class(self, request, class_pk=None):
        school_class = get_object_or_404(Class, pk=class_pk, school__supervisor=request.user)
    ...
    def student_of_class(self, request, student_pk=None):
        student = get_object_or_404(Student, pk=student_pk, school__supervisor=request.user)
...
class ChildrenViewSet(viewsets.ModelViewSet):
    ...
    def create(self, request):
        ...
        if parent.children.count() >= 3:
            return Response({"message": "You can't add more than 3 children"}, status=status.HTTP_400_BAD_REQUEST)
        data = request.data.copy()
...
class TopStudentsView(APIView):
    ...
    def get(self, request, rating_type):
        ...
        if user.is_student:
            ...
            return self._get_top_students_for_child(user, child_id, rating_type, request)
        
        return Response({"detail": "Invalid request. Parent must provide child_id."}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def _get_top_students_for_student(self, user, rating_type, request):
        ...
        if rating_type == 'class':
            if current_student.school_class:
                ...
                return Response({"detail": "Student is not assigned to any class."}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if current_student.school:
                ...
                return Response({"detail": "Student is not assigned to any school."}, status=status.HTTP_400_BAD_REQUEST)
            ...
            if current_student.grade:
                ...
                return Response({"detail": "Student grade is not set."}, status=status.HTTP_400_BAD_REQUEST)
        else:
            return Response({"detail": "Invalid rating type. Use 'class', 'school', or 'global'."}, status=status.HTTP_400_BAD_REQUEST)

        top_students = self._ensure_current_student_in_top_students(current_student, top_students)
    ...
    def _get_top_students_for_child(self, user, child_id, rating_type, request):
        ...
        top_children = self._ensure_current_child_in_top_children(current_child, top_children)
...
class WeeklyProgressAPIView(APIView):
    ...
    def get(self, request):
        ...
        response_data = self._format_weekly_progress_response(daily_progress, start_date)
        
        return Response(response_data, status=status.HTTP_200_OK)

    def _get_task_completions(self, user, child_id, start_date, today):
        if user.is_student:
            return TaskCompletion.objects.filter(user=user, completed_at__date__gte=start_date, completed_at__date__lte=today)
        elif user.is_parent and child_id:
            child = get_object_or_404(Child, pk=child_id, parent=user.parent)
            return TaskCompletion.objects.filter(child=child, completed_at__date__gte=start_date, completed_at__date__lte=today)
        return None
    ...
    def _calculate_daily_progress(self, task_completions, start_date):
        ...
        for task_completion in task_completions:
            day = str(task_completion.completed_at.date())
            if day in daily_progress:
                daily_progress[day] += task_completion.correct * settings.QUESTION_REWARD
        
        return daily_progress

    def _format_weekly_progress_response(self, daily_progress, start_date):
        date_to_day = {str(start_date + timedelta(days=i)): (start_date + timedelta(days=i)).strftime('%A') for i in range(7)}
        return {
            "weekly_progress": [{"day": date_to_day[date], "cups": cups} for date, cups in daily_progress.items()]
        }
...
class CurrentUserView(APIView):
    ...
    def _get_user_data(self, user):
        ...
        if user.is_student:
            data['user'] = self._get_student_data(user, subscription_active, is_free_trial)
        elif user.is_parent:
            data['user'] = self._get_parent_data(user, subscription_active, is_free_trial)
